...
export default class Player extends Generic{
	...
	static artCache = new Map();		// url : image dom element
	...
	constructor(data){
		...
		this.icon_lowerBody = "";			// == || ==
		this.icon_nude = "";				// == || ==
		this.icon_base = "";				// Prepended to all icons
		this.istates = [];					// PlayerIconState objects
	...
	save( full ){
		...
		const out = {
			...
			istates : PlayerIconState.saveThese(this.istates, full),
		};
	...
	getActiveIconStateLayers(){
		...
		for( let layer of this.istates ){
			...
			// Filter active
			if( 
				(ct && !layer.isTriggered()) || 
				(!ct && !layer.validate(evt))
			)continue;
		...
		let all = Array.from(layers.values());
		all.sort((a,b) => {
	...
	async getActiveIcon( useCache = false ){
		...
		else if( ub && !lb && this.icon_upperBody )
			baseLayer = this.icon_upperBody;
	...
	// A text trigger event has been raised where we're the sender or target
	onTextTrigger( event ){
	...
	// Used for 
	static getColorizedCanvas( ){

	}

	// Loads an image from cache, if not in cache, we'll build it
	// Returns false if the image failed to load
	static async fetchCachedImage( url ){

		let cur = this.artCache.get(url);
		// Setup a promise to load it if it's not set
		if( !cur ){
	...
	// returns a canvas where it's imported an image by element, and applied an overlay effect onto it with a color
	static getColoredImage( img, color ){
